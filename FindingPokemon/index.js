...
const abilities = ( ) =>{
    ...
    for(ability of pokemon.abilities){
            alert(`ability name : ${ability.ability.name}
...
const baseExperience = () =>{
    alert(`Base Experirnce ${pokemon.base_experience}`)
}

const height = () =>{
    alert(`Height ${pokemon.height}`)
}

const metalPowderUrl = () =>{
    for(heldItem of pokemon.held_items){
        if(heldItem.item.name === 'metal-powder'){
           alert(`Metal Powder Url : ${heldItem.item.url}`)
        }
    }
}
...
const sprites = () => {
    ...
    Object.keys(sprites).forEach( function(key){
        alert(`${key} : ${sprites[key]}`)

        if(key ==="other" ){
            Object.keys(sprites.other).forEach( function(key1){
                    if(key1 === 'dream_world' || key1 ==='home' || key1==='official-artwork'){      
                        Object.keys(sprites.other[key1]).forEach(function(key1a){
                                alert(`${key1a} : ${sprites.other[key1][key1a]}`)
                        })
                    }
                })
        }else if( key === 'versions'){
            Object.keys(sprites.versions).forEach( function(key1){
                if(key1.includes('generation-')){

                    Object.keys(sprites.versions[key1]).forEach(function(key1a){

                        Object.keys(sprites.versions[key1][key1a]).forEach(function(k){
                            alert(`${key1} ${k} : ${sprites.versions[key1][key1a][k]}`)
                        })
...
const statNames = () =>{
    ...
    for(i=0; i<stats.length-1; i++){
      alert(` stat name : ${stats[i].stat.name}`)
    }
